...
class DriveForward(Node):
    def __init__(self):
        ...
        # Controller timing parameters (matching motion_controller and onnx_infer_ros2.py)
    ...
    def _publish_for(
        self, duration_s: float, msg: VelocityCommandWithHead, log_every_n: int = 50
    ) -> None:
...
def main(args=None):
    ...
    node.get_logger().info(
        "=== Starting Multi-Motion Test (matching onnx_infer_ros2.py behavior) ==="
    )
    node.get_logger().info(
