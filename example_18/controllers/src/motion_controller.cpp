...
CallbackReturn MotionController::on_init()
{
  try
  {
    ...
    get_node()->declare_parameter<std::string>("velocity_command_topic", "~/cmd_head_velocity");
...
CallbackReturn MotionController::on_configure(const rclcpp_lifecycle::State & /*previous_state*/)
{
  ...
  if (param_default_positions.size() == joint_names_.size())
  {
    ...
    RCLCPP_INFO(get_node()->get_logger(),
      "Default joint positions loaded from config (%zu joints)", default_joint_positions_.size());
  }
  else
  {
  ...
  stabilization_delay_ = static_cast<int>(get_node()->get_parameter("stabilization_delay").as_int());
  ...
  test_hardware_commands_publisher_ = get_node()->create_publisher<std_msgs::msg::Float64MultiArray>(
  ...
  inject_yaml_path_subscriber_ = get_node()->create_subscription<std_msgs::msg::String>(
    ...
    { inject_yaml_file_path_ = msg->data; });
  ...
  {
    debug_formatted_observation_publisher_ = get_node()->create_publisher<std_msgs::msg::Float64MultiArray>(
    ...
    debug_processed_action_publisher_ = get_node()->create_publisher<std_msgs::msg::Float64MultiArray>(
      "~/debug/processed_action", rclcpp::QoS(10));
    debug_blended_action_publisher_ = get_node()->create_publisher<std_msgs::msg::Float64MultiArray>(
      "~/debug/blended_action", rclcpp::QoS(10));
    debug_rate_limited_action_publisher_ = get_node()->create_publisher<std_msgs::msg::Float64MultiArray>(
      "~/debug/rate_limited_action", rclcpp::QoS(10));
    debug_prev_motor_targets_publisher_ = get_node()->create_publisher<std_msgs::msg::Float64MultiArray>(
  ...
  const rclcpp::Time & /*time*/, const rclcpp::Duration & period)
{
  ...
  if (velocity_cmd_op.has_value())
  {
    ...
    if (update_count_ % 125 == 0)  // Log every 125 steps (~5 seconds at 25Hz)
    {
      RCLCPP_DEBUG(
        get_node()->get_logger(),
  ...
  else if (update_count_ % 250 == 0)  // Log missing command less frequently
  {
  ...
  if (!default_joint_positions_initialized_)
  {
    ...
    RCLCPP_INFO(get_node()->get_logger(),
      "Default joint positions initialized from sensors (%zu joints)", default_joint_positions_.size());
  }
  ...
  try
  {
    // Debug: Log accelerometer values being used for observation formatting
    ...
    // Debug: Log formatted observation accelerometer values
    if (ignore_interface_data_updates_.load() && model_inputs.size() >= 6)
    {
      RCLCPP_INFO(get_node()->get_logger(),
                  "[DEBUG] Formatted observation accelerometer: [%.6f, %.6f, %.6f]",
                  model_inputs[3], model_inputs[4], model_inputs[5]);
    }
    
    // Publish formatted observation for debugging
  ...
  // Only log after stabilization is complete (when observations are used for control)
  if (update_count_ % 50 == 0 && stabilization_steps_ >= static_cast<size_t>(stabilization_delay_))
  {
  ...
  if (update_count_ % 125 == 0)  // Log every 125 steps (~5 seconds at 25Hz)
  {
    ...
    RCLCPP_DEBUG(
      get_node()->get_logger(),
      "[OBS] Step %zu: max_joint_pos_rel=%.4f, max_joint_vel=%.4f, contacts=[%.1f,%.1f], phase=[%.4f,%.4f]",
      update_count_ + 1, max_joint_pos_rel, max_joint_vel, model_inputs[97], model_inputs[98], phase[0], phase[1]);
  }
  ...
  if (stabilization_steps_ < static_cast<size_t>(stabilization_delay_))
  {
    ...
    if (stabilization_steps_ % 50 == 0)
    {
      RCLCPP_DEBUG(
        get_node()->get_logger(),
        "Stabilization: %zu/%d steps, holding default positions",
  ...
  else
  {
    ...
    if (stabilization_steps_ == static_cast<size_t>(stabilization_delay_))
    {
      RCLCPP_INFO(
        get_node()->get_logger(),
        "Stabilization complete! Starting ONNX control (blend-in: %d steps)",
        blend_in_steps_);
      onnx_active_steps_ = 0;
    }
    ...
    // Publish processed action for debugging
    ...
    double blend_factor = std::min(1.0, static_cast<double>(onnx_active_steps_) / static_cast<double>(blend_in_steps_));
    ...
    {
      joint_commands[i] = 
        (1.0 - blend_factor) * default_joint_positions_[i] +
        blend_factor * joint_commands[i];
    }
    ...
    if (onnx_active_steps_ >= static_cast<size_t>(blend_in_steps_) && reference_motion_blend_factor_ > 0.0)
    {
    ...
    // Publish blended action (after blend-in and reference motion blending) for debugging
  ...
  // Publish prev_motor_targets before rate limiting for debugging
  if (enable_debug_publishing_ && debug_prev_motor_targets_publisher_)
  {
    auto msg = std::make_shared<std_msgs::msg::Float64MultiArray>();
    msg->data = prev_motor_targets_;
  ...
  // Publish rate-limited action for debugging (always publish final joint_commands)
  ...
  // Format: Control: step=900, motor_diff=0.2747 rad, vel_cmd=[0.09, 0.0, 0.0], motor_targets[0]=-0.0837
  if (update_count_ % 50 == 0)  // Log every 50 steps (~1 second at 50Hz)
  {
    RCLCPP_INFO(
      get_node()->get_logger(),
  ...
  // Publish update completion marker (after all debug topics are published)
  ...
  std::shared_ptr<std_srvs::srv::Trigger::Response> response)
{
  ...
  try
  {
    ...
    std::vector<double> joint_commands = action_processor_->process(test_action_output_, default_joint_positions_);
    
    // Apply blend-in factor (assume fully blended for testing, matching normal flow after blend-in completes)
    double blend_factor = 1.0;  // Full blend for testing (matches normal flow when onnx_active_steps_ >= blend_in_steps_)
    for (size_t i = 0; i < joint_commands.size() && i < default_joint_positions_.size(); ++i)
    {
      joint_commands[i] = 
        (1.0 - blend_factor) * default_joint_positions_[i] +
    ...
    // Store current action as previous for next iteration (matches update() line 631)
    previous_action_ = test_action_output_;
    
    // Blend RL actions with reference motion for stability (matches update() lines 633-649)
    ...
    // Apply rate limiting (matches update() lines 659-676)
    // Use training_control_period_ since we don't have actual_control_period in test context
    // Note: This assumes actual_control_period matches training_control_period_ (should be true in normal operation)
    const double max_change = prev_motor_targets_initialized_ ? 
      max_motor_velocity_ * training_control_period_ : 0.0;  // Match normal flow: 0.0 when uninitialized
    if (prev_motor_targets_initialized_ && prev_motor_targets_.size() == joint_commands.size())
    {
      for (size_t i = 0; i < joint_commands.size(); ++i)
      {
        joint_commands[i] = std::clamp(
  ...
  std::shared_ptr<std_srvs::srv::Trigger::Response> response)
{
  ...
  try
  {
    ...
    RCLCPP_INFO(
      get_node()->get_logger(),
      "[RESET] Controller state reset: motor_targets, prev_motor_targets, and action history reset to default positions");
  }
  catch (const std::exception & e)
  {
    response->success = false;
  ...
  std::shared_ptr<std_srvs::srv::Trigger::Response> response)
{
  ...
  RCLCPP_INFO(get_node()->get_logger(), 
              "[INJECT] Received injection state service request");
  ...
  RCLCPP_INFO(get_node()->get_logger(), 
              "[INJECT] Injecting state from YAML: %s", inject_yaml_file_path_->c_str());
  ...
  if (success)
  {
    ...
    response->message = "State injected successfully from YAML file: " + inject_yaml_file_path_.value();
  }
  else
  {
    ...
    RCLCPP_ERROR(get_node()->get_logger(), 
                 "[INJECT] Failed to inject state from YAML file.");
  ...
  std::shared_ptr<std_srvs::srv::Trigger::Response> response)
{
  ...
  response->success = true;
  response->message = "Updates resumed. Controller will process velocity commands and update state.";
  ...
  try
  {
    ...
    if (state["last_action"])
    {
      for (const auto & val : state["last_action"])
        last_action.push_back(val.as<double>());
    }
    ...
    {
      for (const auto & val : state["motor_targets"])
        motor_targets.push_back(val.as<double>());
    }
    ...
    size_t onnx_active_steps = state["onnx_active_steps"] ? state["onnx_active_steps"].as<size_t>() : 0;
    size_t stabilization_steps = state["stabilization_steps"] ? state["stabilization_steps"].as<size_t>() : 0;
    
    // Validate sizes
    if (last_action.size() != N || last_last_action.size() != N || 
    ...
    {
      RCLCPP_INFO(get_node()->get_logger(),
                  "[INJECT] Injected accelerometer: [%.6f, %.6f, %.6f]",
                  interface_data.values[7], interface_data.values[8], interface_data.values[9]);
    }
    ...
    RCLCPP_INFO(get_node()->get_logger(), 
                "State injected from YAML: %s", yaml_file_path.c_str());
    return true;
  }
...
#else
  RCLCPP_ERROR(get_node()->get_logger(), 
               "yaml-cpp not available. Cannot parse YAML file.");
  return false;
#endif
}
