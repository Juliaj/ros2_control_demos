...
TEST_F(MotionControllerMuJoCoTest, WalkingForwardWithAlternatingControlPattern)
{
  ...
  for (int step = 0; step < num_steps; ++step)
  {
    ...
    double phase = static_cast<double>(period_step) / static_cast<double>(pattern_period);  // 0 to 1
    ...
    if (pattern_phase == 0)
    {
      ...
      if (num_joints_ > 2) data_->ctrl[2] += step_amplitude * swing_factor;  // left_hip_pitch (swing forward)
      if (num_joints_ > 3) data_->ctrl[3] += step_amplitude * 0.8 * swing_factor;  // left_knee (bend to lift)
      if (num_joints_ > 4) data_->ctrl[4] -= step_amplitude * 0.5 * swing_factor;  // left_ankle (toes up)
      // Right leg: extend backward to push body forward
      if (num_joints_ > 11) data_->ctrl[11] -= step_amplitude * 0.6 * stance_factor;  // right_hip_pitch (extend back)
      if (num_joints_ > 12) data_->ctrl[12] -= step_amplitude * 0.5 * stance_factor;  // right_knee (straighten)
      if (num_joints_ > 13) data_->ctrl[13] += step_amplitude * 0.3 * stance_factor;  // right_ankle (push off)
    }
    else
    {
      // Right leg swings forward, left leg pushes
      // Right leg: swing forward and up
      if (num_joints_ > 11) data_->ctrl[11] += step_amplitude * swing_factor;  // right_hip_pitch (swing forward)
      if (num_joints_ > 12) data_->ctrl[12] += step_amplitude * 0.8 * swing_factor;  // right_knee (bend to lift)
      if (num_joints_ > 13) data_->ctrl[13] -= step_amplitude * 0.5 * swing_factor;  // right_ankle (toes up)
      // Left leg: extend backward to push body forward
      if (num_joints_ > 2) data_->ctrl[2] -= step_amplitude * 0.6 * stance_factor;  // left_hip_pitch (extend back)
      if (num_joints_ > 3) data_->ctrl[3] -= step_amplitude * 0.5 * stance_factor;  // left_knee (straighten)
      if (num_joints_ > 4) data_->ctrl[4] += step_amplitude * 0.3 * stance_factor;  // left_ankle (push off)
    }
  ...
  EXPECT_GT(forward_distance, -max_backward_drift)
    << "Robot moved significantly backward (" << forward_distance << "m) with alternating control pattern. "
  ...
  {
    std::cout << "  ⚠ Robot moved slightly forward: " << forward_distance << "m (minimal)" << std::endl;
  }
  else
  {
    std::cout << "  ⚠ Robot did not achieve forward motion (drift: " << forward_distance << "m)" << std::endl;
  }
}
