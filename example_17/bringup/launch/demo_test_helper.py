...
class MecanumDriveControllersTest(Node):
    ...
    def publish_cmd_vel(self, delay=0.1):
        ...
        while rclpy.ok():
            ...
            twist_msg.header.stamp = self.get_clock().now().to_msg()
            twist_msg.twist.linear.x = random.uniform(0.0, 0.7)
            twist_msg.twist.angular.z = random.uniform(0.0, 1.0)
            # log the twist message
            self.get_logger().info(f"Publishing twist message to cmd_vel: {twist_msg.twist.linear.x}, {twist_msg.twist.angular.z}")
